...
class Model(torch.nn.Module):
    def __init__(self, args, concat=False):
        ...
        # self.lin2 = torch.nn.Linear(2*self.num_encoder_features,1)#, self.num_classes)
    ...
    def forward(self, x, edge_index, data: Optional[Data] = None):#, titles: Optional[list] = None):
        ...
        # x = gmp(x, data.batch)
        x = torch.stack([x[(data.batch == idx).nonzero().squeeze()[0]] for idx in range(data.num_graphs)])
